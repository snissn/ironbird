...
RUNNER="${RUNNER:-/mnt/fast4tb/tmp/local-report-runner-attribution}"
OUT_ROOT="${OUT_ROOT:-/mnt/fast4tb/ironbird-lowfanout-attribution-$(date -u +%Y%m%dT%H%M%SZ)}"
...
run_one() {
  ...
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    "$workload" "$backend" "$attempt" "$accepted" "$blocks" "$txs" "$seconds" "$successful" \
